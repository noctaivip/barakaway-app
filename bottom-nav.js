(function () {
  ...
  var items = [
    {
      ...
      label: L[0],
      href: file('home'),
      icon: '<svg viewBox="0 0 24 24"><path d="M4 7.5h16"/><path d="M8 3.5v4"/><path d="M16 3.5v4"/><rect x="4" y="5.5" width="16" height="15" rx="3"/><path d="M8 12h.01"/><path d="M12 12h.01"/><path d="M16 12h.01"/><path d="M8 16h.01"/><path d="M12 16h.01"/></svg>'
    },
    {
      key: 'favorites',
      label: L[1],
      href: favoritesPage,
      icon: '<svg viewBox="0 0 24 24"><path d="M12 20.5s-7.2-4.4-7.2-10.4A4.3 4.3 0 0 1 12 7.1a4.3 4.3 0 0 1 7.2 3c0 6-7.2 10.4-7.2 10.4Z"/><path d="M12 7.1v13.4"/></svg>'
    },
    {
      key: 'quran',
      label: L[2],
    ...
    },
    {
      ...
      label: L[3],
      href: file('holidays'),
      icon: '<svg viewBox="0 0 24 24"><path d="M12 3.5c-1.9 1.4-3.1 3.5-3.1 5.9 0 4 3.3 7.3 7.3 7.3 1.4 0 2.7-.4 3.8-1.1A8.6 8.6 0 1 1 12 3.5Z"/><path d="M17.5 4.2l.6 1.6 1.7.1-1.3 1.1.4 1.6-1.4-.9-1.4.9.4-1.6-1.3-1.1 1.7-.1.6-1.6Z"/></svg>'
    },
    {
      key: 'learning',
      label: L[4],
      href: file('learning'),
      icon: '<svg viewBox="0 0 24 24"><rect x="4" y="5" width="16" height="14" rx="3"/><path d="M10 9l5 3-5 3V9Z"/><path d="M7 20h10"/></svg>'
    }
  ];
  ...
  function saveFavorites(items) {
    try {
      localStorage.setItem('barakaway_favorites_v1', JSON.stringify(items || []));
    } catch (e) {}
  }
  ...
  function initFavoriteClickSupport() {
    document.addEventListener('click', function (event) {
      ...
      var type = btn.getAttribute('data-favorite-type') || btn.getAttribute('data-type') || '';
      var text = btn.getAttribute('data-favorite-text') || btn.getAttribute('data-text') || '';
      var id = btn.getAttribute('data-favorite-id') || btn.getAttribute('data-id') || (url + '::' + title);

      window.BarakaWayFavorites.add({ id: id, title: title, url: url, type: type, text: text });
